<?php class Users extends CI_Controller {
		...
		if ($data[0])
			{
				...
				$this->email->set_alt_message(nl2br(trim("Welcome to Studygig!  You are recieving this because you registered at http://studygig.com"))); 
				$this->email->send();
				header('Location: http://studygig.com/index.php/users/dashboard');	
	...
	function createuser() {				
		...
		if($this->form_validation->run('signup')) {
			...
			$this->load->library('email');
				...
				$this->email->subject("Please confirm your Studygig account, ".$username);
			...
			$this->email->message($body); 
				$this->email->set_alt_message('Please confirm your Studygig account by clicking this link (if it doesn\'t work please copy and paste in url): http://studygig.com/index.php/users/verify/'.$verifyhash); 
				$this->email->send();
	...
	function resendVerificationEmail(){
		if($this->form_validation->run('emailreset')){
			...
			$this->email->message($body); 
				$this->email->set_alt_message('Please confirm your Studygig account by clicking this link (if it doesn\'t work please copy and paste in url): http://studygig.com/index.php/users/verify/'.$user->verifyhash); 
				$this->email->send();
  ...
  if ($this->session->userdata('logged_in')) 
		{			
			if($this->form_validation->run('sendInvites')){	
				...
				$this->email->subject($sender->username." has invited you to Studygig!");
